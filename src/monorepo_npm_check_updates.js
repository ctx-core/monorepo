...
export async function monorepo_npm_check_updates(opts = {}) {
    ...
    let current_count = 0;
    const package_name_a = opts.package_name
        ? flatten([opts.package_name])
        : Object.keys(package_name_h_project);
    const stdout_a_async_a = projects.map((project) => _project_stdout_async(project));
    ...
    async function _stdout_async(location = '.') {
        ...
        const update_a = flatten(update_a2);
        if (update_a.length) {
    ...
    async function _project_stdout_async(project) {
    ...
    async function update_dependencies(dependencies, noUpdate = []) {
        ...
        for (let package_name in dependencies) {
            ...
            if (~in_version.indexOf('workspace:'))
                continue;
            if (!valid(coerce(in_version)))
                continue;
            ...
            if (latest_stripped_version
                && compare(coerce(latest_stripped_version) || '', coerce(in_version) || '') > 0) {
...
//# sourceMappingURL=src/monorepo_npm_check_updates.js.map