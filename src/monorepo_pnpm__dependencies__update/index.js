 ...
 */
export async function monorepo_pnpm__dependencies__update(
	params = {}
) {
	/** @type {Record<string, Promise<string>>} */
	const package_name_R_latest_version_promise = {}
	async function package_name_R_latest_version_promise_() {
		/** @type {Record<string, Promise<string>>} */
		const package_name_R_latest_version_promise = {}
		await new Promise(res=>{
			const pnpm_list_recursive = spawn('pnpm', ['list', 'recursive'], {
				stdio: ['pipe', 'pipe', process.stderr]
			})
			line__parse(line=>{
				const word_a = line.split(' ')
				if (word_a.length !== 2) return
				const name_version_word = word_a[0]
				const name_version_match = /^(@?.$)@((\d\.?)+)/.exec(name_version_word)
			}, Readable.toWeb(pnpm_list_recursive.stdout))
			pnpm_list_recursive.on('close', ()=>
				res(true))
		})
		return package_name_R_latest_version_promise
	}
